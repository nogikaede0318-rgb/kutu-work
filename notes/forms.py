...
class BaseWagePeriodFormSet(forms.BaseFormSet):
    def clean(self):
        if any(self.errors):
            return
        months = [form.cleaned_data['effective_month'] for form in self.forms
...
class MonthlyPaidLeaveForm(forms.ModelForm):
    days = forms.IntegerField(label='有給日数', min_value=0, max_value=31,
                              widget=forms.NumberInput(attrs={'min': 0, 'max': 31, 'step': 1}))
    class Meta:
